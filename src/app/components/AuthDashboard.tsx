...
async function downloadClaimFlyerPdf(code: string) {
  const response = await fetch(`/api/flyer/claim/${encodeURIComponent(code)}/pdf`);
  if (!response.ok) throw new Error("Could not download that flyer PDF.");
  const blob = await response.blob();
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `bayblaze-claim-flyer-${code.toLowerCase().replace(/[^a-z0-9-]+/g, "-")}.pdf`;
  anchor.click();
